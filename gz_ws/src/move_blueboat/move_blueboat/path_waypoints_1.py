...
class CoordinateApp:
    ...
    def get_coordinates(self, event):
        ...
        if self.mode == "start":
            ...
            self.canvas.create_oval(coord[0]-self.node_radius, coord[1]-self.node_radius,
                                    coord[0]+self.node_radius, coord[1]+self.node_radius, fill="black")
        ...
        for i in range(1000):
            ...
            if not self.is_colliding(nearest_node, new_node):
                ...
                if self.distance(new_node, goal) < self.step_size:
                    nodes.append(goal)
                    return self.extract_path(nodes, goal)
        return None
    ...
    def is_colliding(self, from_node, to_node):
        for obstacle in self.obstacles:
            if self.distance_to_line(obstacle, from_node, to_node) < self.node_radius:
                return True
        return False
    ...
    def extract_path(self, nodes, goal):
        path = [goal]
        current_node = goal
        while current_node != self.start:
            current_node = self.get_nearest_node(nodes, current_node)
