...
class WayASVController(Node):
    def __init__(self):
        ...
        self.gps_subscription = self.create_subscription(String, '/gps_true', self.gps_callback, qos_profile=custom_qos, callback_group=self.callback_group)
        ...
        self.waypoint_subscription = self.create_subscription(Float64MultiArray, '/waypoints', self.waypoint_callback, qos_profile=custom_qos, callback_group=self.callback_group)
    ...
    def gps_callback(self, msg):
        ...
        if self.previous_position != (0, 0):  # Skip if it's the first GPS reading
            ...
            if self.yaw_offset is None:
                self.yaw_offset = self.current_yaw_odometry - raw_yaw_navsat
    ...
    def navigate_to_waypoint(self):
        ...
        heading_error_odometry = self.normalize_angle(bearing_to_waypoint - self.current_yaw_odometry)
        ...
        self.get_logger().info(f"Heading Error (NavSat): {heading_error_navsat:.2f}, Heading Error (Odometry): {heading_error_odometry:.2f}")
