...
class CoordinateApp:
    ...
    def open_zone_parameter_window(self):
        ...
        for zone_idx in range(4):
            tk.Label(self.zone_window, text=f"Zone {zone_idx+1}").grid(row=zone_idx*5, column=0, sticky='w')

            tk.Label(self.zone_window, text="Ocean Current Direction").grid(row=zone_idx*5 + 1, column=0, sticky='w')
            ocean_current_direction_entry = tk.Entry(self.zone_window)
            ocean_current_direction_entry.grid(row=zone_idx*5 + 1, column=1)
            ocean_current_direction_entry.insert(0, str(self.zones[zone_idx]['ocean_current']['direction']))

            tk.Label(self.zone_window, text="Ocean Current Force").grid(row=zone_idx*5 + 2, column=0, sticky='w')
            ocean_current_force_entry = tk.Entry(self.zone_window)
            ocean_current_force_entry.grid(row=zone_idx*5 + 2, column=1)
            ocean_current_force_entry.insert(0, str(self.zones[zone_idx]['ocean_current']['force']))

            tk.Label(self.zone_window, text="Wave Direction").grid(row=zone_idx*5 + 3, column=0, sticky='w')
            wave_direction_entry = tk.Entry(self.zone_window)
            wave_direction_entry.grid(row=zone_idx*5 + 3, column=1)
            wave_direction_entry.insert(0, str(self.zones[zone_idx]['wave']['direction']))

            tk.Label(self.zone_window, text="Wave Height").grid(row=zone_idx*5 + 4, column=0, sticky='w')
            wave_height_entry = tk.Entry(self.zone_window)
            wave_height_entry.grid(row=zone_idx*5 + 4, column=1)
            wave_height_entry.insert(0, str(self.zones[zone_idx]['wave']['height']))

            tk.Label(self.zone_window, text="Wind Direction").grid(row=zone_idx*5 + 5, column=0, sticky='w')
            wind_direction_entry = tk.Entry(self.zone_window)
            wind_direction_entry.grid(row=zone_idx*5 + 5, column=1)
            wind_direction_entry.insert(0, str(self.zones[zone_idx]['wind']['direction']))

            tk.Label(self.zone_window, text="Wind Speed").grid(row=zone_idx*5 + 6, column=0, sticky='w')
            wind_speed_entry = tk.Entry(self.zone_window)
            wind_speed_entry.grid(row=zone_idx*5 + 6, column=1)
        ...
        save_button.grid(row=20, column=0, columnspan=2, sticky='nsew')
        ...
        for zone_idx, zone in enumerate(self.zones):
            ...
            for i in range(zone_x, zone_x + zone_width, self.grid_size):
                for j in range(zone_y, zone_y + zone_height, self.grid_size):
                    ...
                    self.draw_arrow(x, y, zone['ocean_current'], "blue")
                    self.draw_arrow(x, y, zone['wave'], "green")
                    self.draw_arrow(x, y, zone['wind'], "red")
