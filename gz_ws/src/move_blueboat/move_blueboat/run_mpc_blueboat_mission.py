...
class RobotController(Node):
    ...
    def control_robot(self, current_position, current_yaw):
        # MPC calculation
        surge_force, sway_force, yaw_moment = self.calculate_mpc(current_position, current_yaw)

        # Translate MPC outputs to thruster commands
        left_thrust, right_thrust = self.translate_to_thrust(surge_force, sway_force, yaw_moment)

        # Publish thrust commands
        self.publish_thrust(left_thrust, right_thrust)

    def calculate_mpc(self, current_position, current_yaw):
        ...
        for k in range(self.N):
            ...
            ref_state = np.array([self.smooth_x[k % len(self.smooth_x)], 
                                self.smooth_y[k % len(self.smooth_y)], 
