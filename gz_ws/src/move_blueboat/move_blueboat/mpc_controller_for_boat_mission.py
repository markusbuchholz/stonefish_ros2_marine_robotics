...
class MPCASVController(Node):
    def __init__(self):
        super().__init__('mpc_asv_mission_controller')
    ...
    def navigate_to_waypoint(self):
        ...
        if self.state == 'move_to_waypoint':
            if self.current_waypoint_index < len(self.waypoints_x) - 1:
    ...
    def solve_mpc(self, start_index):
        ...
        T1 = m.MV(value=0, lb=-10, ub=10)  # Adjusted bounds based on the thrust curve
        T2 = m.MV(value=0, lb=-10, ub=10)  # Adjusted bounds based on the thrust curve
        T1.STATUS = 1
        T2.STATUS = 1
        ...
        self.get_logger().info(f"Reference X values: {ref_x.value}")
        self.get_logger().info(f"Reference Y values: {ref_y.value}")

        # Mapping T1 and T2 to emulate Fx and Mz directly
        Fx = T1 + T2
        Mz = (T1 - T2) * d

        H = 0.0 #3.0 # high of wave
        Vw = 0.0 #1  # Wind speed (m/s)
        Vc = 0.0 #0.5  # Speed of the current (m/s)
        
        ##### Model of waves #####
        rho_water = 1000  # Density of water (kg/m^3)
        g = 9.81  # Acceleration due to gravity (m/s^2)
        L, B, T = 2, 2, 0.5  # Length, Breadth, and Draft of the vehicle
        A, Lambda, omega_e, phi = H, 25000.0, 0.5, 0
        beta = np.pi / 4
...
def main(args=None):
    ...
    mpc_asv_mission_controller = MPCASVController()
    executor = MultiThreadedExecutor()
    executor.add_node(mpc_asv_mission_controller)

    try:
        mpc_asv_mission_controller.get_logger().info('ASVController node is running')
        executor.spin()
    except KeyboardInterrupt:
        mpc_asv_mission_controller.get_logger().info('ASVController node is shutting down')
    finally:
        mpc_asv_mission_controller.destroy_node()
        rclpy.shutdown()
