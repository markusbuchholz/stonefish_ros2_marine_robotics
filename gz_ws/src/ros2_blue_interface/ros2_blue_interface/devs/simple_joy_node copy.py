...
class JoystickNode(Node):
    ...
    async def handle_joy(self, joy_msg):
        ...
        for index, state in enumerate(joy_msg.buttons):
            if str(index) in self.btn_action:
                if state == 1 and self.btn_action[str(index)][1] == False:
                    await self.handle_button_action(str(index))
    ...
    async def handle_button_action(self, btn_index):
        """Handle asynchronous button actions."""
        action_func = self.btn_action[btn_index][0]
        await action_func()
