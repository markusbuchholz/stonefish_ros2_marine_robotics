...
OpenGLOcean::OpenGLOcean(GLfloat size)
{
    ...
    */
    ...
    //oceanShaders["background"]->AddUniform("sfactor", ParameterType::FLOAT);
    ...
    std::cout << "LOADING JERLOV EMBED " << (char*)scattering << std::endl;
#else
    ...
    std::cout << "Scattering data (hexadecimal):" << std::endl;
    std::cout << "Scattering data (hexadecimal):" << std::endl;
    /*for (size_t i = 0; i < sizeof(scattering); ++i) {
        for (int j = 0; j < 3; ++j) {
            int value = static_cast<int>(scattering[i][j]); // Scale to [0, 255] range
            //std::cout << value << " " ;
        }
            //std::cout << std::endl;
    }*/
...
void OpenGLOcean::setWaterType(GLfloat t)
{
    ...
    if(!isJerlovUsed()){
            // check
	    lightScattering=glm::vec3(0.0159309, 0.0220465, 0.0321737);
    }else{
    if(fPart > 0.f) 
    {
...
void OpenGLOcean::DrawBackground(OpenGLCamera* cam)
{
    ...
    //std::cout <<" SFACTOR VALUE " << getSfactor() << "  " << getLightAttenuation()[0]<< std::endl;
    //oceanShaders["background"]->SetUniform("sfactor", getSfactor());
...
std::map<std::string, GLSLShader*>& OpenGLOcean::getOceanShaders() {
        return oceanShaders;
}

bool OpenGLOcean::isJerlovUsed(){
     return use_jerlov;
}
        
void OpenGLOcean::setJerlovUse(bool j){
     use_jerlov=j;
}
}
