	...
	: OpenGLLight(position, radius, color, lum)
{
    ...
    clipSpace = glm::mat4();
	GLfloat near = (getSourceRadius()/4)/tanf(coneAngle/2.f);
    ...
    {
        vt.pos = glm::vec3(sinf(i/(GLfloat)slices*M_PI*2.f)*(getSourceRadius()/4), -cosf(i/(GLfloat)slices*M_PI*2.f)*(getSourceRadius()/4), 0.0);
        m->vertices.push_back(vt);
    }
    //faces
    for(unsigned int i=0; i<slices; ++i)
