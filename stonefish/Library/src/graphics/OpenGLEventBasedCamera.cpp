...
{  
    ...
    cameraLogLumTex = OpenGLContent::GenerateTexture(GL_TEXTURE_2D, glm::uvec3(viewportWidth, viewportHeight, 0), 
    ...
    cameraEventTex[0] = OpenGLContent::GenerateTexture(GL_TEXTURE_2D, eventsTextureSize, 
                                                     GL_RG32I, GL_RG_INTEGER, GL_INT, NULL, FilteringMode::NEAREST, false);
    // --> Last event timestamp (in ns, with polarization)
    cameraEventTex[1] = OpenGLContent::GenerateTexture(GL_TEXTURE_2D, glm::uvec3(viewportWidth, viewportHeight, 0), 
                                                     GL_R32I, GL_RED_INTEGER, GL_INT, NULL, FilteringMode::NEAREST, false);
    // --> Crossings
    cameraEventTex[2] = OpenGLContent::GenerateTexture(GL_TEXTURE_2D, glm::uvec3(viewportWidth, viewportHeight, 0), 
                                                     GL_R32F, GL_RED, GL_FLOAT, NULL, FilteringMode::NEAREST, false);
    // Allocate buffer to store the event counter
    GLuint zero = 0;
    glGenBuffers(1, &cameraEventCounter);
    glBindBuffer(GL_ATOMIC_COUNTER_BUFFER, cameraEventCounter); 
...
{
    glDeleteTextures(1, &cameraLogLumTex);
    glDeleteTextures(3, cameraEventTex);
    glDeleteBuffers(1, &cameraEventCounter);

    if(camera != nullptr)
        glDeleteBuffers(1, &cameraPBO);    
}

ViewType OpenGLEventBasedCamera::getType()
{
...
void OpenGLEventBasedCamera::ComputeOutput(double dt)
{
    if(initialized)
    {
        ...
        // Compute events
        glBindBufferBase(GL_ATOMIC_COUNTER_BUFFER, 0, cameraEventCounter);
        glBindImageTexture(TEX_POSTPROCESS1, renderColorTex[lastActiveRenderColorBuffer], 0, GL_FALSE, 0, GL_READ_ONLY, GL_RGBA32F);
        glBindImageTexture(TEX_POSTPROCESS2, cameraLogLumTex, 0, GL_FALSE, 0, GL_READ_WRITE, GL_R32F);
        glBindImageTexture(TEX_POSTPROCESS3, cameraEventTex[0], 0, GL_FALSE, 0, GL_WRITE_ONLY, GL_RG32I);
        glBindImageTexture(TEX_POSTPROCESS4, cameraEventTex[1], 0, GL_FALSE, 0, GL_READ_WRITE, GL_R32I);
        glBindImageTexture(TEX_POSTPROCESS5, cameraEventTex[2], 0, GL_FALSE, 0, GL_READ_WRITE, GL_R32F);
    ...
    else
    {
        ...
        glBindImageTexture(TEX_POSTPROCESS1, renderColorTex[lastActiveRenderColorBuffer], 0, GL_FALSE, 0, GL_READ_ONLY, GL_RGBA32F);
        glBindImageTexture(TEX_POSTPROCESS2, cameraLogLumTex, 0, GL_FALSE, 0, GL_WRITE_ONLY, GL_R32F);
        glBindImageTexture(TEX_POSTPROCESS4, cameraEventTex[1], 0, GL_FALSE, 0, GL_WRITE_ONLY, GL_R32I);
        glBindImageTexture(TEX_POSTPROCESS5, cameraEventTex[2], 0, GL_FALSE, 0, GL_WRITE_ONLY, GL_R32F);
...
void OpenGLEventBasedCamera::DrawLDR(GLuint destinationFBO, bool updated)
{
    ...
    if(display)
    {
        ...
        OpenGLState::BindTexture(TEX_POSTPROCESS1, GL_TEXTURE_2D, cameraEventTex[1]);
        OpenGLState::BindFramebuffer(destinationFBO);
		OpenGLState::Viewport(0,0,viewportWidth,viewportHeight);
        eventVisualizeShader->Use();
        eventVisualizeShader->SetUniform("texEventTimes", TEX_POSTPROCESS1);
    ...
    {
        OpenGLState::BindTexture(TEX_POSTPROCESS1, GL_TEXTURE_2D, cameraEventTex[0]);
        glBindBuffer(GL_PIXEL_PACK_BUFFER, cameraPBO);
...
void OpenGLEventBasedCamera::Init()
{
    ...
    eventVisualizeShader = new GLSLShader("eventVisualize.frag");
    eventVisualizeShader->AddUniform("texEventTimes", ParameterType::INT);
    //eventVisualizeShader->AddUniform("texCrossings", ParameterType::INT);
}
