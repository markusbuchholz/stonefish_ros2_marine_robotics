...
void IMGUI::DrawRect2(GLfloat x, GLfloat y, GLfloat w, GLfloat h, glm::vec4 color)
{
    y = windowH - y;
...
void IMGUI::DrawCircle(GLfloat x, GLfloat y, GLfloat radius, glm::vec4 color)
{
    y = windowH - y;
    
    radius = radius / windowW * 2.f;
    x = x / windowW * 2.f - 1.f;
    y = y / windowH * 2.f - 1.f;

    const int numSegments = 1000; // Number of segments to approximate the circle

    GLfloat cx = x;
    GLfloat cy = y;

    GLfloat twicePi = 2.0f * M_PI;
    GLfloat step = twicePi / numSegments;
    ...
    for (int i = 0; i < numSegments; ++i) {
        ...
        GLfloat x1 = cx + radius * cos(angle1);
        GLfloat y1 = cy + radius * sin(angle1);
        GLfloat x2 = cx + radius * cos(angle2);
        GLfloat y2 = cy + radius * sin(angle2);
...
void IMGUI::DrawRoundedRectAngle(GLfloat x, GLfloat y, GLfloat w, GLfloat h, glm::vec4 color, float rotationAngle)
{
    // Adjust width and height if they are smaller than four times the corner radius
    w = w < 4.f * CORNER_RADIUS ? 4.f * CORNER_RADIUS : w;
    h = h < 4.f * CORNER_RADIUS ? 4.f * CORNER_RADIUS : h;
    y = windowH - y; // Invert y-coordinate
    
    // Calculate corner size based on window dimensions
    glm::vec2 cs(CORNER_RADIUS / windowW * 2.f, CORNER_RADIUS / windowH * 2.f);
    w = w / windowW * 2.f;
    h = h / windowH * 2.f;
    x = x / windowW * 2.f - 1.f;
    ...
    // Rotate rectangle vertices based on the rotation angle
    glm::mat2 rotationMatrix(cos(glm::radians(rotationAngle)), -sin(glm::radians(rotationAngle)),
                              sin(glm::radians(rotationAngle)), cos(glm::radians(rotationAngle)));

    for (int i = 0; i < 24; ++i) {
        // Rotate each vertex position
        glm::vec2 rotatedPosition = rotationMatrix * glm::vec2(rectData[i][0], rectData[i][1]);
        rectData[i][0] = rotatedPosition.x;
        rectData[i][1] = rotatedPosition.y;
    }
...
void IMGUI::DrawRoundedRect2(GLfloat x, GLfloat y, GLfloat w, GLfloat h, glm::vec4 color)
{
    w = w < 2.5f*CORNER_RADIUS ? 2.5f*CORNER_RADIUS : w;
    h = h < 2.5f*CORNER_RADIUS ? 2.5f*CORNER_RADIUS : h;
    y = windowH - y;
...
void IMGUI::DrawTriangle(GLfloat x, GLfloat y, GLfloat w, GLfloat h, glm::vec4 color)
{
    y = windowH - y;
