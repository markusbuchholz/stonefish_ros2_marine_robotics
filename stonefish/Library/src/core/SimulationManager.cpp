...
ColorCamera* SimulationManager::getColorCamera(){
       for(int i=0; i<sensors.size();i++){
           if(sensors[i]->getName()=="trackball")
              return (ColorCamera*)sensors[i];
       }
}

void SimulationManager::setAnnotation(bool value){
     annotation=value;
}

bool SimulationManager::isAnnotationEnabled(){
     return annotation;
}
...
Sensor* SimulationManager::getSensor(size_t index)
{
...
std::string SimulationManager::CreateSegmentationLook(const std::string& name, Color color, GLfloat specular, GLfloat shininess, GLfloat reflectivity, const std::string& albedoTextureName)
{
    if(SimulationApp::getApp()->hasGraphics())
        return ((GraphicalSimulationApp*)SimulationApp::getApp())->getGLPipeline()->getContent()->CreateSimpleLook(name, color.rgb, shininess, reflectivity, GLfloat(0.0f),albedoTextureName);
    else
        return "";
}
...
std::vector<Actuator*> SimulationManager::getActuators(){
     return actuators;
}
