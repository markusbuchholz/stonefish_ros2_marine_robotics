...
bool ScenarioParser::ParseStatic(XMLElement* element)
{
    ...
    //Goal
    element->QueryAttribute("goal",&goal);
...
bool ScenarioParser::ParseRobot(XMLElement* element)
{
    ...
    std::ofstream ofs(std::string(std::filesystem::current_path())+"/"+robotName+".txt", std::ofstream::trunc);
    ...
    bool luma=false;
    Transform origin_luma;
    
    item = element->FirstChildElement("comm");
    if(item!=nullptr){
       const char* type =nullptr;
       if(item->QueryStringAttribute("type",&type)!=XML_SUCCESS){
          log.Print(MessageType::ERROR, "Luma of robot '%s' not properly defined!", robotName.c_str());
	delete robot;
	return false;
       }
       std::cout<<"LUMA CHECK1 "<< type << std::endl;
       if(strcmp(type,"luma")==0){
       
          //LUMA-CHECK
          
            std::cout<<"LUMA CHECK2"<<std::endl;
            XMLElement* item2;
            if((item2 = item->FirstChildElement("origin")) == nullptr || !ParseTransform(item2, origin_luma)){
            log.Print(MessageType::ERROR, "Luma of robot '%s' not properly defined!", robotName.c_str());
            }
            SolidEntity* link=nullptr;
            BodyPhysicsSettings phy;
            ...
            Vector3 xyz2 = Vector3(0.0,0.0,0.0);
            ...
            link=new Cylinder(robot->getName()+"/Luma", phy, 0.03, 0.126, origin, "Steel", "Neutral", Scalar(-1));
            link->ScalePhysicalPropertiesToArbitraryMass(0.01);
            links.push_back(link);
            luma=true;
       }
    ...
    item = element->FirstChildElement("tether");
    if(item != nullptr)
    {
		double length=0.f;
		const char* look =nullptr;
		...
		if(item->QueryAttribute("length", &length) != XML_SUCCESS){
			log.Print(MessageType::ERROR, "Tether of robot '%s' not properly defined!", robotName.c_str());
			delete robot;
			return false;
		}
		...
		if(item->QueryStringAttribute("material", &material) != XML_SUCCESS){
			log.Print(MessageType::ERROR, "Tether of robot '%s' not properly defined!", robotName.c_str());
			material="Neutral";
		}
		if(item->QueryAttribute("mass_per_sphere", &mass) != XML_SUCCESS){
			log.Print(MessageType::ERROR, "Tether of robot '%s' not properly defined, missing mass_per_sphere!", robotName.c_str());
			return false;
		}
		if(item->QueryAttribute("radius", &radius) != XML_SUCCESS){
			log.Print(MessageType::ERROR, "Tether of robot '%s' not properly define, missing radius sphere: radius!", robotName.c_str());
			return false;
		}
		if(item->QueryAttribute("dist_between_spheres", &dist) != XML_SUCCESS){
			log.Print(MessageType::ERROR, "Tether of robot '%s' not properly defined, missing distance between sphere: dist_between_spheres!", robotName.c_str());
			return false;
		}
		BodyPhysicsSettings phy;
		phy.mode = BodyPhysicsMode::SUBMERGED;
		Vector3 xyz = Vector3(0.0,0.0,0.0);
		Vector3 xyz2 = Vector3(-1,0.0,0.0);
		...
		robot->DefineLinks(baseLink, links, true);
		if(luma)
			robot->DefineFixedJoint("JointLuma", robot->getName()+"/Vehicle", robot->getName()+"/Luma", origin_luma);
	}
    //---- Joints ----
    ...
    while(item != nullptr)
    {
        ...
        else
        {
            ...
            if(comm->getType()==CommType::OPTIC){
		Vector3 rpy = Vector3(-1.57,0.0,0.0);
		Vector3 xyz2 = Vector3(-0.013, 0.016, -0.02);
		Transform origin2 = Transform(Quaternion(rpy.z(), rpy.y(), rpy.x()), xyz2);
               robot->AddLinkActuator(((Luma*)comm)->getLights()[0], robotName + "/" + std::string(linkName), origin2);
		xyz2 = Vector3(-0.01, 0.016, -0.015);
		origin2 = Transform(Quaternion(rpy.z(), rpy.y(), rpy.x()), xyz2);
               robot->AddLinkActuator(((Luma*)comm)->getLights()[1], robotName + "/" + std::string(linkName), origin2);
               
               
               xyz2 = Vector3(-0.007, 0.016, -0.01);
               origin2 = Transform(Quaternion(rpy.z(), rpy.y(), rpy.x()), xyz2);
               robot->AddLinkActuator(((Luma*)comm)->getLights()[2], robotName + "/" + std::string(linkName), origin2);
               
               xyz2 = Vector3(0.013, 0.016, -0.02);
               origin2 = Transform(Quaternion(rpy.z(), rpy.y(), rpy.x()), xyz2);
               robot->AddLinkActuator(((Luma*)comm)->getLights()[3], robotName + "/" + std::string(linkName), origin2);
               
               xyz2 = Vector3(0.01, 0.016, -0.015);
               origin2 = Transform(Quaternion(rpy.z(), rpy.y(), rpy.x()), xyz2);
               robot->AddLinkActuator(((Luma*)comm)->getLights()[4], robotName + "/" + std::string(linkName), origin2);
               
               xyz2 = Vector3(0.007, 0.016, -0.01);
               origin2 = Transform(Quaternion(rpy.z(), rpy.y(), rpy.x()), xyz2);
               robot->AddLinkActuator(((Luma*)comm)->getLights()[5], robotName + "/" + std::string(linkName), origin2);
               
               xyz2 = Vector3(-0.02, 0.016, 0.01);
               origin2 = Transform(Quaternion(rpy.z(), rpy.y(), rpy.x()), xyz2);
               robot->AddLinkActuator(((Luma*)comm)->getLights()[6], robotName + "/" + std::string(linkName), origin2);
               
               xyz2 = Vector3(-0.015, 0.016, 0.0075);
               origin2 = Transform(Quaternion(rpy.z(), rpy.y(), rpy.x()), xyz2);
               robot->AddLinkActuator(((Luma*)comm)->getLights()[7], robotName + "/" + std::string(linkName), origin2);

               xyz2 = Vector3(-0.01, 0.016, 0.005);
               origin2 = Transform(Quaternion(rpy.z(), rpy.y(), rpy.x()), xyz2);
               robot->AddLinkActuator(((Luma*)comm)->getLights()[8], robotName + "/" + std::string(linkName), origin2);
               
               
               xyz2 = Vector3(0.02, 0.016, 0.01);
               origin2 = Transform(Quaternion(rpy.z(), rpy.y(), rpy.x()), xyz2);
               robot->AddLinkActuator(((Luma*)comm)->getLights()[9], robotName + "/" + std::string(linkName), origin2);

               xyz2 = Vector3(0.015, 0.016, 0.0075);
               origin2 = Transform(Quaternion(rpy.z(), rpy.y(), rpy.x()), xyz2);
               robot->AddLinkActuator(((Luma*)comm)->getLights()[10], robotName + "/" + std::string(linkName), origin2);
               
               xyz2 = Vector3(0.01, 0.016, 0.005);
               origin2 = Transform(Quaternion(rpy.z(), rpy.y(), rpy.x()), xyz2);
               robot->AddLinkActuator(((Luma*)comm)->getLights()[11], robotName + "/" + std::string(linkName), origin2);
               
               
               xyz2 = Vector3(0.0, 0.016, 0.02);
               origin2 = Transform(Quaternion(rpy.z(), rpy.y(), rpy.x()), xyz2);
               robot->AddLinkActuator(((Luma*)comm)->getLights()[12], robotName + "/" + std::string(linkName), origin2);
               
               xyz2 = Vector3(0.0, 0.016, 0.015);
               origin2 = Transform(Quaternion(rpy.z(), rpy.y(), rpy.x()), xyz2);
               robot->AddLinkActuator(((Luma*)comm)->getLights()[13], robotName + "/" + std::string(linkName), origin2);
               
               xyz2 = Vector3(0.0, 0.016, 0.01);
               origin2 = Transform(Quaternion(rpy.z(), rpy.y(), rpy.x()), xyz2);
               robot->AddLinkActuator(((Luma*)comm)->getLights()[14], robotName + "/" + std::string(linkName), origin2);
   ...
   //Watchdog
    ...
    switch(act->getType())
    {
        ...
        case ActuatorType::SUCTION_CUP:
        {
            ...
            if(act->getType()==ActuatorType::THRUSTER){
               Thruster* t = static_cast<Thruster*>(act);
               t->writeAllocMatrix(std::string(std::filesystem::current_path())+"/"+robot->getName()+".txt");
            }
...
Sensor* ScenarioParser::ParseSensor(XMLElement* element, const std::string& namePrefix)
{
    ...
    if(element->QueryAttribute("voltage", &rate) != XML_SUCCESS)
       voltage = Scalar(0);
       
    if(element->QueryAttribute("watt", &rate) != XML_SUCCESS)
        watt = Scalar(0);
        
    if(element->QueryAttribute("duty_cycle", &rate) != XML_SUCCESS)
        duty_cycle = Scalar(0);
    ...
    else if(typeStr == "ais")
    {
        ...
        AIS* ais = new AIS(sensorName, rate, history);
        
        if((item = element->FirstChildElement("identification")) != nullptr)    
        {
         
         const char* ID = nullptr;
         if(item->QueryStringAttribute("id", &ID) != XML_SUCCESS){
         ...
         const char* vessel_type = nullptr;
         if(item->QueryStringAttribute("vessel_type", &vessel_type) != XML_SUCCESS){
            log.Print(MessageType::ERROR, "Specs of sensor '%s' not properly defined!", sensorName.c_str());
            return nullptr;
         }else{
             ais->setVesselType(vessel_type);
         }
         
        }else{
           log.Print(MessageType::ERROR, "Identification of sensor '%s' not properly defined!", sensorName.c_str());
            return nullptr;}
    ...
    else if(typeStr == "opticalflowcamera")
    {
        if(!isGraphicalSim())
        {
        ...
        {
            GLfloat maxV;
            if(item->QueryAttribute("maximum_velocity", &maxV) == XML_SUCCESS)
                ofcam->setDisplaySettings(maxV);
        }

        sens = ofcam;
    }
    else if(typeStr == "eventbasedcamera")
    {
        if(!isGraphicalSim())
        {
        ...
        if((item = element->FirstChildElement("specs")) == nullptr 
            ...
            || item->QueryAttribute("resolution_y", &resY) != XML_SUCCESS
            || item->QueryAttribute("horizontal_fov", &hFov) != XML_SUCCESS
            || item->QueryAttribute("positive_contrast_threshold", &Cp) != XML_SUCCESS
            || item->QueryAttribute("negative_contrast_threshold", &Cm) != XML_SUCCESS
        ...
        if((item = element->FirstChildElement("rendering")) != nullptr) 
        {
            ...
            if(c == 0)
            {
                ...
                cam = new EventBasedCamera(sensorName, resX, resY, hFov, Cp, Cm, Tref, rate);
            }
            else
                cam = new EventBasedCamera(sensorName, resX, resY, hFov, Cp, Cm, Tref, rate, minDist, maxDist);
        }
        else
            cam = new EventBasedCamera(sensorName, resX, resY, hFov, Cp, Cm, Tref, rate);
        ...
        if((item = element->FirstChildElement("noise")) != nullptr)    
        {
            ...
            Scalar sigmaCm(0);
            int c = 0;
            if(item->QueryAttribute("positive_contrast_threshold", &sigmaCp) == XML_SUCCESS)
                ++c;
            if(item->QueryAttribute("negative_contrast_threshold", &sigmaCm) == XML_SUCCESS)
                ++c;
            if(c == 0)
                log.Print(MessageType::WARNING, "Noise of sensor '%s' not properly defined - using defaults.", sensorName.c_str());
            else
                cam->setNoise(sigmaCp, sigmaCm);
        }
        sens = cam;
    }
...
Comm* ScenarioParser::ParseComm(XMLElement* element, const std::string& namePrefix)
{
    ...
    else if(typeStr == "luma")
    {
        std::cout<<"found LUMA"<<std::endl;
        Scalar comm_speed;
        Scalar range;
        unsigned int cId = 0;
        bool occlusion = true;
        ...
        std::cout<<"Connecting LUMA"<<std::endl;
        comm = new Luma(commName, devId, range, comm_speed);
        for(int i=0;i<15;i++){
            Light* light = new Light(namePrefix+"/LUMAX", 0.0005,15, Color::RGB(0, 0, 1), 25000);
            ((Luma*)comm)->addLight(light);
        }
        //comm->Connect(cId);
        std::cout<<"Connected LUMA"<<std::endl;
...
bool ScenarioParser::ParseTether(XMLElement* element)
{
    ...
    Scalar radius(0);
    if(element->QueryAttribute("radius", &radius) != XML_SUCCESS)
    {        
        log.Print(MessageType::ERROR, "Length of tether missing!");
        return false;
    }
    Scalar dist(0);
    if(element->QueryAttribute("dist_between_spheres", &dist) != XML_SUCCESS)
    {        
        log.Print(MessageType::ERROR, "Length of tether missing!");
        return false;
    }
    ...
    if(fix != nullptr)
    {
        ...
        log.Print(MessageType::INFO, "Tether not created between '%s' and %s'.", nameA, nameB);
    }
    return false;
}
...
bool ScenarioParser::ParseColorMap(XMLElement* element, ColorMap& cm)
{
    ...
    if(element->QueryStringAttribute("colormap", &colorMap) == XML_SUCCESS)
    {
        ...
        else if(colorMapStr == "monochrome")
            cm = ColorMap::MONOCHROME;
        else
        {
