...
void Thruster::writeAllocMatrix(std::string filename){
        //Get transforms
        Transform solidTrans = attach->getCGTransform();
        Transform thrustTrans = attach->getOTransform() * o2a;
        Vector3 relPos = thrustTrans.getOrigin() - solidTrans.getOrigin();
        std::ofstream allocationMatrix(filename,std::ios_base::app);
        btQuaternion q = thrustTrans.getRotation();
        ...
        btScalar zw = q.z() * q.w();
        btScalar ww = q.w() * q.w();
        // Construct rotation matrix from quaternion elements
        ...
        // Unit Force vector transformed into body frame.
        Vector3 unitForce = rotationMatrix * Vector3(1,0,0);
        // Unit Toquer vector transformed into body frame.
        Vector3 unitTorque = relPos.cross(unitForce);
        // Write to the file
        allocationMatrix << getName() << "\n";
        allocationMatrix << unitForce.x()<<"  "<< unitForce.y()<<"  "<< unitForce.z()<<"  ";
        allocationMatrix << unitTorque.x()<<"  "<< unitTorque.y()<<"  "<< unitTorque.z()<< "\n";
        allocationMatrix.close();
}
